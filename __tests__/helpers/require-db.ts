 ...
 * every golden contract test (the database is not in the checkout — the
 * 0-byte stub was untracked in #48), making the contract gate vacuous: the
 * same defect class found in the Slovak-law-mcp PR #50 review. CI sets
 * REQUIRE_DB=1 after building the DB from the committed seeds; if the DB is
 * ever missing again under that flag, the suite must fail loud, not skip.
 */
