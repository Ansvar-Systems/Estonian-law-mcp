#!/usr/bin/env tsx
/**
 * Estonian Law MCP -- Ingestion Pipeline
 *
 * Fetches Estonian legislation from the Sejm ELI API (api.sejm.gov.pl).
 * The Sejm (Estonian Parliament) provides free public access to all legislation
 ...
 * License: Estonian legislation is public domain under Art. 4 of the Copyright Act
 */
...
import { parseEstonianHtml, KEY_ESTONIAN_ACTS, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function fetchAndParseActs(acts: ActIndexEntry[], skipFetch: boolean): Promise<void> {
  console.log(`\nProcessing ${acts.length} Estonian Acts from api.sejm.gov.pl...\n`);
  ...
  for (const act of acts) {
    ...
    try {
      ...
      const parsed = parseEstonianHtml(html, act);
  ...
  console.log(`  License:     Public domain (Art. 4 Estonian Copyright Act)`);
...
async function main(): Promise<void> {
  ...
  console.log('Estonian Law MCP -- Ingestion Pipeline');
  console.log('====================================\n');
  ...
  const acts = limit ? KEY_ESTONIAN_ACTS.slice(0, limit) : KEY_ESTONIAN_ACTS;
