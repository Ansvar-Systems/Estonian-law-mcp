#!/usr/bin/env tsx
/**
 * Paid-tier database builder for Estonian Law MCP server.
 *
...
CREATE TABLE IF NOT EXISTS case_law (
  ...
  source TEXT NOT NULL DEFAULT 'riigiteataja',
...
CREATE TABLE IF NOT EXISTS preparatory_works (
  ...
  draft_uuid TEXT,
  session_uuid TEXT,
  koosseisu_nr INTEGER,
  source TEXT NOT NULL DEFAULT 'riigikogu',
  inserted_at TEXT DEFAULT (datetime('now'))
);
...
CREATE INDEX IF NOT EXISTS idx_prep_works_draft_uuid ON preparatory_works(draft_uuid);
...
function buildPaidTier(): void {
  ...
  console.log('Building paid-tier extensions for Estonian Law MCP...\n');
  ...
  const updateMeta = db.transaction(() => {
    ...
    upsertMeta.run('jurisdiction', 'EE');
    upsertMeta.run('paid_tables', 'case_law,case_law_full,preparatory_works,preparatory_works_full,agency_guidance');
    upsertMeta.run('source_case_law', 'riigiteataja');
    upsertMeta.run('source_prep_works', 'riigikogu_api');
    upsertMeta.run('source_agency', 'aki.ee,konkurentsiamet.ee,fi.ee');
  });
  updateMeta();
