#!/usr/bin/env tsx
/**
 * Estonian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.riigiteataja.ee';
const PORTAL_NAME = 'Riigi Teataja';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/estonian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Estonian Law MCP — Data Freshness Check');
