/**
 * HTML parser for Estonian legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Estonian legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Estonian statute.
 *
 ...
 *   </div>
 */
export function parseEstonianHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Estonian acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Estonian legal text.
 *
 * Estonian definitions typically use patterns like:
