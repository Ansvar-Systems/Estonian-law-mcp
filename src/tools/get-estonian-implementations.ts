/**
 * get_estonian_implementations — Find Estonian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetEstonianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface EstonianImplementationResult {
  document_id: string;
...
export async function getEstonianImplementations(
  db: InstanceType<typeof Database>,
  input: GetEstonianImplementationsInput,
): Promise<ToolResponse<EstonianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as EstonianImplementationResult[];
