/**
 * search_eu_implementations — Search EU directives/regulations referenced by Estonian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_estonian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as estonian_statute_count
  ...
  if (input.has_estonian_implementation) {
    sql += ' HAVING estonian_statute_count > 0';
  }
