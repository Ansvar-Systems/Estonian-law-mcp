/**
 * Tool registry for Estonian Law MCP Server.
...
import { getEstonianImplementations, type GetEstonianImplementationsInput } from './get-estonian-implementations.js';
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Estonian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Estonian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Estonian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Estonian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Estonia?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Estonian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Estonian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Estonian statute references or aligns with. ' +
      'Estonia is an EU member state and Estonian laws implement EU directives/regulations ' +
      '(e.g., IKS implements GDPR, Cybersecurity Act transposes NIS2). ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Estonian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_estonian_implementations',
    description:
      'Find all Estonian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Estonian statutes. ' +
      'Estonia transposes EU directives into national law as an EU Member State.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Estonian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_estonian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Estonian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Estonian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Estonian statute or provision. ' +
      ...
      'Estonia is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Estonian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_estonian_implementations':
          result = await getEstonianImplementations(db, args as unknown as GetEstonianImplementationsInput);
          break;
